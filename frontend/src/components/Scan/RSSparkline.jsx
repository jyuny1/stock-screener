...
function RSSparkline({
  ...
  const tooltipText = useMemo(() => {
    ...
    const sign = change >= 0 ? '+' : '';
    return `${tooltipLabel} ${trendText} (${sign}${change.toFixed(1)}% over ${tooltipWindow})`;
  }, [trend, chartData, tooltipTitle, tooltipLabel, tooltipWindow]);
  ...
  const chart = (
    <Box sx={{ width, height, cursor: disableTooltip ? 'default' : 'pointer' }}>
  ...
  if (disableTooltip) return chart;
...
export default memo(RSSparkline, (prevProps, nextProps) => {
  const sameDisplayProps = (
    prevProps.trend === nextProps.trend &&
    prevProps.width === nextProps.width &&
    prevProps.height === nextProps.height &&
    prevProps.tooltipTitle === nextProps.tooltipTitle &&
    prevProps.tooltipLabel === nextProps.tooltipLabel &&
    prevProps.tooltipWindow === nextProps.tooltipWindow &&
    prevProps.disableTooltip === nextProps.disableTooltip
  );
  // Deep compare data arrays
  if (prevProps.data === nextProps.data) return sameDisplayProps;
