"""Build market-scoped foundation update bundles for static-site workflows."""
...
def _default_output_dir() -> Path:
    return repo_root() / ".tmp" / "foundation-update"
...
def _default_bundle_name(market: str, published_run) -> str:
    ...
    return f"foundation-update-{market.lower()}-{as_of}-{revision}.json.gz"


def _default_latest_manifest_name(market: str) -> str:
    return f"foundation-update-latest-{str(market or '').strip().lower()}.json"
...
def _write_step_summary(market: str, summary: dict[str, Any]) -> None:
    ...
    lines = [
        f"## Foundation Update Bundle: {market}",
        "",
        "| Metric | Value |",
        "| --- | --- |",
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            pass
        # Assumption: the ``Seed prior foundation update bundle`` step in
        # foundation-update-data.yml is the only writer of CN rows in the
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Market code to build the foundation update bundle for.",
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--bundle-name",
        default=None,
        help="Bundle asset filename. Defaults to foundation-update-<market>-<YYYYMMDD>-<revision>.json.gz",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="US foundation-update universe mode. Use optionable for static deployments.",
    )
    parser.add_argument(
    ...
    _write_step_summary(market, summary)
    print(f"Foundation update bundle complete for {market}:")
